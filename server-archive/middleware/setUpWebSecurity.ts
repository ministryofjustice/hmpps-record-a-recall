...
export default function setUpWebSecurity(): Router {
  ...
  const fontSrc = ["'self'"]
  const formAction = [
    `'self' ${config.apis.hmppsAuth.externalUrl} ${config.applications.digitalPrisonServices.url} ${config.applications.courtCasesReleaseDates.url} ${config.applications.adjustments.url}`,
  ]
