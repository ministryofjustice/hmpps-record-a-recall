...
export default class RecallBaseController extends PrisonerDetailsController {
  ...
  protected formError(fieldName: string, type: string): any {
    return {
      key: fieldName,
      type,
    }
  ...
  checkJourneyProgress(req: any, res: Response, next: NextFunction) {
  ...
  locals(req: any, res: Response): Record<string, unknown> {
    ...
    const hasMultipleOverlappingUALTypeRecall: boolean =
      mockSessionModel.get(sessionModelFields.HAS_MULTIPLE_OVERLAPPING_UAL_TYPE_RECALL) || false
    ...
    const lv = (req as any).journeyModel?.attributes?.lastVisited || (req.session as any)?.lastVisited
