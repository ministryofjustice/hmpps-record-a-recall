...
describe('ClientCredentialsRedisTokenStore', () => {
  ...
  it('stores tokens under the systemToken prefix with the given TTL', async () => {
    ...
    expect(mockClient.set).toHaveBeenCalledWith('clientCredentialsToken:user1', 'abc123', { EX: 3600 })
  })
  ...
  it('retrieves tokens from under the systemToken prefix', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith('clientCredentialsToken:user1')
  })
  ...
  describe('evictToken', () => {
    it('deletes the token using the same key format used by setToken/getToken', async () => {
      ...
      expect(mockClient.del).toHaveBeenCalledWith('clientCredentialsToken:user1')
    })
  ...
  it('keys entries independently per user', async () => {
    ...
    expect(mockClient.set).toHaveBeenNthCalledWith(1, 'clientCredentialsToken:user1', 'tokenA', { EX: 3600 })
    expect(mockClient.set).toHaveBeenNthCalledWith(2, 'clientCredentialsToken:user2', 'tokenB', { EX: 3600 })
  })
})
