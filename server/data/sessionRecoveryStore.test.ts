...
describe('sessionRecoveryStore', () => {
  ...
  describe('when redis is enabled', () => {
    ...
    it('saves session data under a username+nomsId key with a 30 minute TTL', async () => {
      const session = { recallJourneys: { 'journey-1': { id: 'journey-1' } } } as unknown as Session &
        Partial<SessionData>
    ...
    it('restores and deletes the stored session data when present', async () => {
      mockClient.get.mockResolvedValueOnce(JSON.stringify({ recallJourneys: { 'journey-1': { id: 'journey-1' } } }))
      ...
      expect(result).toEqual({ recallJourneys: { 'journey-1': { id: 'journey-1' } } })
    })
