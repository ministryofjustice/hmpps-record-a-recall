...
export const ensureInRecallJourney = asyncMiddleware(async (req: Request<PersonJourneyParams>, res, next) => {
  ...
  if (!req.session.recallJourneys[journeyId]) {
    logger.warn(
      `Recall journey (${journeyId}) not found in session for user ${res.locals.user?.username}. Returning to start of journey.`,
    )
