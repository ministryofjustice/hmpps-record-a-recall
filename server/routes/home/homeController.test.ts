...
describe('GET', () => {
  ...
  const response = await request(app).get(
    `/person/${nomsId}?sortBy=APPEARANCE_DATE_ASC`,
  )
  ...
  expect(
    recallCards.eq(0).find('.govuk-summary-card__title').text().trim(),
  ).toStrictEqual('Recorded on 18 Jan 2019')

  expect(
    recallCards.eq(1).find('.govuk-summary-card__title').text().trim(),
  ).toStrictEqual(
    'Recorded on 26 Feb 2020 at Moorland (HMP & YOI)',
  )

  expect(
    recallCards.eq(2).find('.govuk-summary-card__title').text().trim(),
  ).toStrictEqual(
    'Recorded on 19 Mar 2021 at Kirkham (HMP)',
  )
})
