...
describe('GET', () => {
  it('should unsupported recall type', async () => {
    // Given
    ...
    expect(text).toContain('The recall type cannot be saved')
    expect(text).toContain('Update recall type')
  })

  it('should return to start of journey if not found in session', async () => {
    await request(app)
      .get(`/person/${nomsId}/recall/create/${uuidv4()}/unsupported-recall-type`)
      .expect(302)
