...
router.get('/populate-stored-recall', async (req, res, next) => {
  try {
    const reqWithWizard = req as unknown as FormWizard.Request
    ...
    const courtCaseOptions = await getCourtCaseOptionsFromRas(reqWithWizard, res)
    reqWithWizard.sessionModel.set(sessionModelFields.COURT_CASE_OPTIONS, courtCaseOptions)

    // Configure the controller (loads recall data and adjustments)
    await controller.configure(reqWithWizard, res, next)

    // Call the locals method to populate session data
    // This sets up all the session fields from the stored recall
    const locals = controller.locals(reqWithWizard, res)

    // Store the locals in res.locals for compatibility with templates
    Object.assign(res.locals, locals)

    // Redirect to edit summary page
    res.redirect('/recall/edit/edit-summary')
  } catch (error) {
