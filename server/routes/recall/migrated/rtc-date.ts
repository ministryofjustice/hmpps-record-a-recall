...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let nextStep = '/check-sentences'

      if (hasMultipleUALTypeRecallConflictingFromSession(req) || hasMultipleConflictingFromSession(req)) {
        nextStep = '/conflicting-adjustments-interrupt'
      } else if (isManualCaseSelectionFromSession(req)) {
        nextStep = '/manual-recall-intercept'
      } else if (getEligibleSentenceCountFromSession(req) === 0) {
        nextStep = '/no-sentences-interrupt'
      }
