...
router.get('/select-court-case', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (Array.isArray(formData.selectedCourtCases)) {
      selectedCases = formData.selectedCourtCases
    } else if (formData.selectedCourtCases) {
      selectedCases = [formData.selectedCourtCases as string]
    }
    ...
    const convictionDate = currentCase.date ? formatDateStringToDDMMYYYY(currentCase.date) : 'Unknown date'
...
router.post('/select-court-case', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (nextCaseIndex >= activeCases.length) {
      ...
      const allCourtCases = (req.session.formData.courtCases || []) as Array<{
        caseId?: string
        sentences?: Array<{ sentenceTypeUuid?: string }>
      }>
      const selectedCourtCases = allCourtCases.filter(c => selectedCases.includes(c.caseId || ''))
      const hasUnknownSentences = selectedCourtCases.some(courtCase =>
        courtCase.sentences?.some(sentence => sentence.sentenceTypeUuid === SENTENCE_TYPE_UUIDS.UNKNOWN_PRE_RECALL),
      )
    ...
    return res.redirect(`/person/${prisoner.prisonerNumber}/record-recall/select-court-case`)
  } catch (error) {
